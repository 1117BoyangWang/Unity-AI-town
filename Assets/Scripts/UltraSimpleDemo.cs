...
public class UltraSimpleDemo : MonoBehaviour
{
    ...
    void TriggerDialogue()
    {
        ...
        if (aliveAgents.Count >= 2)
        {
            ...
            if (agent1 != agent2)
            {
                StartDialogue(agent1, agent2);
            }
        }
    }
    
    void StartDialogue(SimpleAgent speaker, SimpleAgent listener)
    {
        string dialogue = GenerateDialogue(speaker, listener);
        speaker.lastDialogue = dialogue;
        
        Debug.Log($"💬 {speaker.name} 对 {listener.name} 说: \"{dialogue}\"");
        
        // 监听者的回应
        string response = GenerateResponse(listener, speaker, dialogue);
    ...
    string GenerateDialogue(SimpleAgent speaker, SimpleAgent listener)
    {
        // 基于代理状态和性格生成对话
    ...
    string GenerateResponse(SimpleAgent responder, SimpleAgent original, string originalMessage)
    {
        string[] positiveResponses = { "谢谢关心", "我很好", "一起加油" };
        string[] concernedResponses = { "你也要照顾好自己", "我们互相帮助", "团结一致" };
        string[] professionalResponses = { "作为{0}，我觉得我们应该...", "我的经验告诉我...", "让我来帮助大家" };
        ...
        {
            return string.Format(professionalResponses[Random.Range(0, professionalResponses.Length)], responder.profession);
        }
        else
        {
    ...
    void OnGUI()
    {
        GUI.Box(new Rect(10, 10, 250, 130), "AI小镇演示");
